...
| `en` | ✅ | 英語タグ（重複チェックキー） |
| `jp` | ✅ | 日本語訳 |
| `category` | ✅ | カテゴリ ID **または日本語ラベル**（`pose` / `ポーズ` どちらでも可） |
| `subcategory` | - | サブカテゴリ ID **または日本語ラベル**（`pose_hands` / `手・腕` どちらでも可）。空欄時はカテゴリ末尾のサブカテゴリに追加 |
| `section` | - | セクションラベル（日本語可。存在しなければ自動作成）。空欄時は「スプレッドシート取り込み」 |
